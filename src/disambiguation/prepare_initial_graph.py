...
class InitialGraph:
    ...
    def extract_strong_relations(self):
        strong_relations = filter(lambda visit_count: visit_count["visits"] > self.threshold_visit, self.node_visit_counts)
    ...
    def increment_visits(self, picked_relation):
        ...
        self.node_visit_counts.append({ "relation": picked_relation["relation"], "node1": self.current_node, "node2": picked_relation["node2"] })
    ...
    def random_walk_with_restart(self):
        ...
        for node in related_nodes:
            ...
            weight = self.neo4j_mgr.get_triangle_weight(self.current_node, node2)
            weight_sum += weight
