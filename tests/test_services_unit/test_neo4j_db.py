...
def test_get_related_nodes_weighted_princeton():
    ...
    assert len(nodes) == 5
    assert any([node["node2"] == "http://plwordnet.pwr.wroc.pl/wordnet/synset/294435" for node in nodes])
    assert any([node["node2"] == "http://plwordnet.pwr.wroc.pl/wordnet/synset/294403" for node in nodes])
    ...
    assert any([node["node2"] == "http://plwordnet.pwr.wroc.pl/wordnet/synset/294435" and node["weight"] == 2 for node in nodes])
    assert any([node["node2"] == "http://plwordnet.pwr.wroc.pl/wordnet/synset/294403" and node["weight"] == 2 for node in nodes])
